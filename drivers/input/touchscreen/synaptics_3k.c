		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ts_data->noise_information) {
		i2c_syn_read(ts_data->client, get_address_base(ts_data, 0x01, DATA_BASE)
			+ ((ts_data->finger_support * 21 + 15) / 4), &data[0], 1);
		count += sprintf(buf + count, "\nIM: %5d", data[0]);
	}
	return count;
}
				...
				for (i = 0; i < ts->finger_support; i++) {
					...
					if (((finger_pressed | finger_release_changed) & BIT(i)) == BIT(i)) {
						...
						if (!ts->finger_count && !ts->pre_finger_data[0][0]
							&& (jiffies > (ts->timestamp + 15 * HZ) || (finger_data[i][1] > ts->raw_base
							&& abs(ts->pre_finger_data[i + 1][1] - finger_data[i][1]) > ts->raw_ref))) {
							ts->pre_finger_data[0][0] = 2;
							printk(KERN_INFO "Touch Calibration Confirmed\n");
						} else if (!ts->finger_count)
